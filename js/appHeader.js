import {handleWeatherByGeoLocation} from '../js/geoLocation.js';
import {getWeatherData} from '../js/api.js';
import {cToF, fToC, resetWeatherContent} from '../js/helper.js';
...
export const createHeader = (city) => {
    ...
    unitsC.addEventListener('click', () => {
        ...
        const convertTemp = fToC(+temperature.textContent)
        temperature.textContent = Math.round(convertTemp)
    })
    ...
    unitsF.addEventListener('click', () => {
        ...
        const temperature = document.querySelector('.weather__temperature')
        const convertTemp = cToF(+temperature.textContent)
        temperature.textContent = Math.round(convertTemp)
    })
